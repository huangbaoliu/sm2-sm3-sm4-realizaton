 ...
 *
 */
static void sm4_one_round(unsigned int sk[32], unsigned char input[16], unsigned char output[16] )
{
    unsigned int i = 0;
    unsigned int X[36];
    ...
    while(i<32)
    {
        X[i+4] = sm4F(X[i], X[i+1], X[i+2], X[i+3], sk[i]);
        i++;
    }
...
/*encode and decode round key revert*/
/*
